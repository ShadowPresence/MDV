...
// $('#list').on('pageinit', function(it){
var list = function(it) {
	var setAnchor = $('<a>'),
		setText = $('<p>'),
		setList = $('<li>'),
		setDate = $('<p>');
...
// need to get href to be dynamic
	setAnchor.attr("href", "#detail"); // <li><a href="detail.html"></a></li>
...
$('#edit').on('click', function(){
	console.log("The edit button was pushed");
});

$('#save').on('click', function(){
	console.log("The form submit button was pushed");
});

$('#detail').on('pageinit', function(){
	console.log("The detail button was pushed");
	$('#compHead').empty();
	$.getJSON('xhr/data.json', function(y){
		//This loops through each object(item) in the json.data file.
		for (var i = 0; i < y.inspections.length; i++) {
			that = y.inspections[i];
			console.log(that);
		};
		var xyz = $('<hr />');
		$('#compHead').append($('<p>').append($('<strong>').html("Date: "), that.date));
		$('#compHead').append($('<h3>').html(that.company));
		$('#compHead').append($('<p>').append($('<strong>').html("Account #:"), that.account));
